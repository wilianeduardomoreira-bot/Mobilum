...
import { View, Employee, Room, RoomStatus, MaintenanceTicket, ActivityLog, ActivityType, Transaction, Product } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (logsData) {
          setActivityLogs(logsData.map(l => ({ ...l, timestamp: new Date(l.timestamp) })));
        }
  ...
  const generateDefaultRooms = () => {
    ...
    floorConfig.forEach(floor => {
      for (let i = floor.range[0]; i <= floor.range[1]; i++) {
        ...
        if (floor.type === 'Standard') bedType = i % 2 === 0 ? 'casal' : 'duplo';
        else if (floor.type === 'Luxo') bedType = i % 2 === 0 ? 'casal' : 'triplo';
        
        generatedRooms.push({
          id: i,
          number: roomNum.toString(),
          type: floor.type as any,
  ...
  const generateDefaultEmployees = () => {
    setEmployees([
      { id: '1', name: 'Carlos Silva', email: 'carlos.gerente@hotel.com', phone: '(11) 99999-1234', username: 'carlos.silva', password: '123', role: 'Gerente', permissions: { suites: true, dashboard: true, calendar: true, reports: true, maintenance: true, cashier: true, chat: true, settings: true } },
      { id: '2', name: 'Ana Souza', email: 'ana.recep@hotel.com', phone: '(11) 98888-5678', username: 'ana.souza', password: '123', role: 'Recepcionista', permissions: { suites: true, dashboard: false, calendar: true, reports: false, maintenance: true, cashier: true, chat: true, settings: false } },
      { id: '3', name: 'Maria Oliveira', email: 'maria.limpeza@hotel.com', phone: '(11) 97777-1111', username: 'maria.o', password: '123', role: 'Arrumadeira', permissions: { suites: true, dashboard: false, calendar: false, reports: false, maintenance: false, cashier: false, chat: true, settings: false } }
    ]);
  };

  const addLog = async (type: ActivityType, action: string, description: string, user: string = 'Sistema') => {
    ...
    try {
      await supabase.from('activity_logs').insert({
        type, action, description, user, timestamp: new Date().toISOString()
      });
    } catch(e) { console.error('Error logging to Supabase', e); }
  };
  ...
  const [shiftData, setShiftData] = useState({ operator: '', startingFloat: 0, startTime: new Date(), shiftName: '' });

  const handleOpenShift = (operator: string, float: number, shiftName: string) => {
    setShiftData({ operator, startingFloat: float, startTime: new Date(), shiftName });
    setIsShiftOpen(true);
  ...
  const handleCloseShift = () => {
    ...
    setShiftData({ operator: '', startingFloat: 0, startTime: new Date(), shiftName: '' });
  };
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case View.SUITES: 
        return <SuitesView employees={employees} rooms={rooms} setRooms={setRooms} tickets={tickets} setTickets={setTickets} products={products} addLog={addLog} addTransaction={addTransaction} />;
  ...
  return (
    <div className="flex h-screen w-full bg-[#F5F5F7] dark:bg-slate-950 overflow-hidden font-sans text-gray-900 dark:text-slate-100 flex-col md:flex-row transition-colors duration-300">
      <div className="md:hidden h-16 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 flex items-center justify-between px-4 shrink-0 z-30 transition-colors duration-300">
        <button onClick={() => setIsMobileMenuOpen(true)} className="p-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg">
          <Menu size={24} />
        </button>
      ...
      <div className={`fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-in-out h-full md:relative md:translate-x-0 md:block ${isMobileMenuOpen ? 'translate-x-0 w-64' : '-translate-x-full w-64 md:w-auto'}`}>
        <Sidebar currentView={currentView} onChangeView={handleViewChange} onCloseMobile={() => setIsMobileMenuOpen(false)} isDarkMode={isDarkMode} toggleTheme={toggleTheme} onLogout={handleLogout} currentUser={currentUser} />
      </div>
